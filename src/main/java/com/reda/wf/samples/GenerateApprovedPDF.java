...
import com.reda.wf.REDAWfModel;



public class GenerateApprovedPDF extends REDAExecutionJavaDelegate {

	private static Logger LOGGER = Logger.getLogger(GenerateApprovedPDF.class);
		
	/**
	 * 	var supplierPO = bpm_package.children[1];
		logger.log("supplier PO = " + supplierPO.name);
		var pdfDoc = supplierPO.transformDocument("application/pdf");
		var oldName = pdfDoc.name.replace(".pdf", "");
		pdfDoc.name = oldName + "-Approved.pdf";
	 */
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		try {
			...
			//get 2nd document attached to the wf
			NodeRef supplierPO = childRefList.get(1).getChildRef();
			NodeRef parentFolder = nodeService.getPrimaryParent(supplierPO).getParentRef();
			
			//create a new node for PDF
			Map<QName, Serializable> contentProps = new HashMap<QName, Serializable>();
			
			//append "Approved" sufix
			contentProps.put(ContentModel.PROP_NAME, nodeService.getProperty(supplierPO, ContentModel.PROP_NAME) + "-Approved");
			contentProps.put(ContentModel.PROP_TITLE, "Approved Supplier Purchase Order");
					
			NodeRef approvedSupplierPoPDF = nodeService.createNode(parentFolder, ContentModel.ASSOC_CONTAINS, ContentModel.ASSOC_CONTAINS, REDAWfModel.PO_CASE_TYPE_NAME, contentProps).getChildRef();
			
			ContentService contentService = getServiceRegistry().getContentService();
			ContentReader pptReader = contentService.getReader(supplierPO, ContentModel.PROP_CONTENT);
			ContentWriter pdfWriter = contentService.getWriter(approvedSupplierPoPDF, ContentModel.PROP_CONTENT, true);

			// it is mandatory to set mimetype
			pdfWriter.setMimetype(MimetypeMap.MIMETYPE_PDF); // new mimetype
			pdfWriter.setEncoding(pptReader.getEncoding()); // original encoding
			
			ContentTransformer pptToPdfTransformer = contentService.getTransformer(pptReader.getMimetype(), MimetypeMap.MIMETYPE_PDF);
